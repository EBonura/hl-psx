...
/// Talk-monster event aligned with the opt-in GoldSrc `HLREF|talk_event`
/// probe. `voice` is the local streamed-dialogue id (255 when irrelevant),
/// and goal is populated for move-away schedule selection/completion.
pub fn talk(
    map_tick: u16,
    event: &str,
    index: usize,
    ...
    line.field_u32("voice", voice as u32);
    line.field_u32("duration", duration as u32);
    line.field_i32("x", pos[0]);
    line.field_i32("y", pos[1]);
    line.field_i32("z", pos[2]);
    line.field_i32("goal_x", goal[0]);
    line.field_i32("goal_y", goal[1]);
    line.field_i32("goal_z", goal[2]);
    line.finish();
}
...
/// Assignment-time FTriangulate candidate. `side` is 0 for GoldSrc's
/// right-first candidate and 1 for left; result bit0/bit1 report whether the
/// start-to-apex and apex-to-goal hull chords were clear. Reference-only.
pub fn script_detour(
    map_tick: u32,
    index: u16,
    blocked_dist: i32,
    ring: u8,
    ...
    let mut line = Line::new("detour");
    common(&mut line, map_tick);
    line.field_u32("index", index as u32);
    line.field_i32("blocked_dist", blocked_dist);
    line.field_u32("ring", ring as u32);
    line.field_u32("side", side as u32);
    line.field_i32("x", apex[0]);
    line.field_i32("y", apex[1]);
    line.field_i32("z", apex[2]);
    line.field_u32("result", result as u32);
    line.finish();
}
...
/// Original dispenser lifecycle, diagnostic builds only.
pub fn beverage(
    map_tick: u32,
    event: &str,
    owner: usize,
    ...
    let mut line = Line::new("beverage");
    common(&mut line, map_tick);
    line.field_str("event", event);
    line.field_u32("owner", owner as u32);
    line.field_i32("stock", stock as i32);
    line.field_u32("skin", skin as u32);
    line.field_i32("x", pos[0]);
    line.field_i32("y", pos[1]);
    line.field_i32("z", pos[2]);
    line.field_u32("health", health as u32);
