...
///
/// The brush is 150 units wide (half-width 75); a 32-unit player hull can
/// remain supported slightly beyond that edge. 93 also matches the last
/// grounded GoldSrc sample on the c0a0e exit walk.
pub const RIDER_LOCAL_Z_LIMIT: i32 = 93;
...
mod tests {
    ...
    fn rider_local_z_keeps_the_goldsrc_edge_seat() {
        assert_eq!(clamp_rider_local_z(-94), -93);
        assert_eq!(clamp_rider_local_z(-89), -89);
        assert_eq!(clamp_rider_local_z(0), 0);
        assert_eq!(clamp_rider_local_z(93), 93);
        assert_eq!(clamp_rider_local_z(94), 93);
