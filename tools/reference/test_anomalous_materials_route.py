...
from hlinput import ACTION_JUMP, ACTION_USE, decode, encode, read_route, write_route
...
class AnomalousMaterialsRouteTests(unittest.TestCase):
    def test_route_shape_and_milestone_ticks_are_stable(self) -> None:
        route = build_route()
        route.validate()
        self.assertEqual([segment.map for segment in route.segments], ["c1a0", "c1a0d"])
        self.assertEqual([segment.next_map for segment in route.segments], ["c1a0d", ""])
        self.assertEqual([segment.total_ticks for segment in route.segments], [616, 741])
        self.assertEqual([len(segment.runs) for segment in route.segments], [14, 35])
        self.assertEqual(decode(encode(route)), route)

    def test_required_interactions_are_not_lost(self) -> None:
        c1a0, c1a0d = (segment.expand() for segment in build_route().segments)
        ...
        self.assertEqual(sum(bool(sample.actions & ACTION_JUMP) for sample in c1a0d), 40)
    ...
    def test_generator_output_is_a_readable_tape(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            output = Path(directory) / "anomalous-materials-to-hev.hlinput"
