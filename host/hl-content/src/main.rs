...
fn usage() -> ! {
    eprintln!(
        "usage:\n\
         \x20 hl-content menu <valve-dir> <out-dir>\n\
         \x20 hl-content music <valve-dir> <out-dir>\n\
         \x20 hl-content sfx <sound-dir> <out.psxa>\n\
         \x20 hl-content voices <valve-dir> <map-list> <out-dir>\n\
         \x20 hl-content sound-audit <valve-dir> <map-list>\n\
...
fn main() -> Result<()> {
    ...
    match arg(&args, 1) {
        "menu" => menu::build(Path::new(arg(&args, 2)), Path::new(arg(&args, 3))),
        "music" => audio::build_music(Path::new(arg(&args, 2)), Path::new(arg(&args, 3))),
        "sfx" => audio::build_sfx(Path::new(arg(&args, 2)), Path::new(arg(&args, 3))),
        "voices" => audio::build_voices(
            Path::new(arg(&args, 2)),
            arg(&args, 3),
            Path::new(arg(&args, 4)),
        ),
        "sound-audit" => audio::audit_map_sounds(Path::new(arg(&args, 2)), arg(&args, 3)),
